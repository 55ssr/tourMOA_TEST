...
$(document).ready(function(){
    ...
    $("#openday_init").change(function(){
    	var nm =  $("#openday_init option:selected").text();
		$("label[for='select_bx_st']").text(nm);
	...
	$("form[name='searchFrm'] #searchCondition").change(function(){
		var nm = $("form[name='searchFrm'] #searchCondition option:selected").text();
		$("label[for='prd_sel01']").text(nm);
	});
	$("#adult").change(function(){
		$("label[for='prd_sel06']").text($("#adult option:selected").text());
	});
	$("#child").change(function(){
		$("label[for='prd_sel07']").text($("#child option:selected").text());
	});
	$("#baby").change(function(){
		$("label[for='prd_sel08']").text($("#baby option:selected").text());
	});
});//ready 닫기
 // 왕복 / 편도 교체
 ...
 // 국내 출발
	...
	$(window).scroll(function(){
		if ($(this).scrollTop() > 230) {
			...
			$("#sidemenu").css({top: "223px", right: "-3px", height: "760px", position: "absolute"});
		}
	});
...
<div id="main" style="height:540px">
	...
	<div id="quickmenu">
			...
			<ul>
				<li class="find">
					<p class="finder">
						<i class="fa fa-tripadvisor" aria-hidden="true"></i>여행상품 찾기
						<span id="dvide"></span>
					</p>
					<div class="find-content">
						...
						<div class="quickDiv" style="padding-top: 15px">상품유형</div>
						...
						<form name="searchFrm" method="post" action="/search/searchKeyword.do">
							<div class="quickDiv" style="padding-top: 10px; height:35px;">
								<div class="select_bx_st w100">
									<label for="prd_sel01">유형을 선택하세요.</label>
									<select name="searchCondition" id="searchCondition">
										<option value="">유형을 선택하세요</option>
										<option value="PKG">해외패키지</option>
										<option value="FIT">자유여행</option>
										<option value="DMT">국내여행</option>
										<option value="HYM">허니문</option>
										<option value="GLF">골프</option>
										<option value="CRS">크루즈</option>
										<option value="HYL">성지순례</option>
										<option value="FBD">부산/대구출발</option>
									</select>
									<input type="hidden" name="categoryDepth1" id="categoryDepth1" value="">
									<input type="hidden" name="categoryDepth2" id="categoryDepth2" value="">
									<input type="hidden" name="categoryName2" id="categoryName3" value="">
								</div>
							</div>
						<div class="quickDiv">여행지역/호텔명</div>
						<div class="quickDiv" style="height: 35px">
							<div class="input_ph w100">
								<input type="text" id="searchKeyword" name="searchKeyword" 
									class="input_txt" placeholder="여행지역/호텔명을 입력하세요">
								<button type="button" class="input_reset" title="지역명 입력취소"></button>
							</div>
							<!-- 
							<input class="find-content-inside" type="text" id="searchKeyword" name="searchKeyword"
							placeholder="여행지역/호텔명을 입력하세요" />
							 -->
						</div>
						<div class="quickDiv">출발일월</div>
						<div class="quickDiv" style="height: 35px;">
							<div class="input_ph w100">
								<input type="text" id="departDate" name="departDate" 
								class="input_txt find-content-inside w100" value="" placeholder="출발일월을 선택하세요" readonly />
							</div>
						</div>
						<div class="quickDiv">
				...
				<li class="airportfind">
					<span class="preparing" style="display:none">준비중 입니다</span>
					<p class="airportfinder">
						<i class="fa fa-plane" aria-hidden="true"></i>항공권 찾기
					</p>
					<div class="airportfind-content">
						<div class="quickDiv clearfix">
							<input type="button" id="airlineBtn" name="airlineBtn"
								class="airportfind_airlineBtn" value="해외항공권" />
							<input type="button" id="domesticBtn" name="domesticBtn"
								class="airportfind_domesticBtn" value="국내항공권" />
						</div>
						<div class="quickDiv">
							<!-- 왕복/편도 설정 id ="air_flightWay" VALUE = 왕복 : R 편도 : O  -->
							<input type="radio"  id="air_flightWay" name="air_flightWay" onclick="fn_initForm('RT')" checked="checked" /> 왕복 
							<input type="radio"	 id="air_flightWay" name="air_flightWay" onclick="fn_initForm('OW')" /> 편도
						</div>
						<div class="quickDiv">출발</div>
						<div class="quickDiv clearfix">
						
						
							<div class="select_bx_st st01">
								<label for="prd_sel04">인천</label>
								<select id="domDepCtyCode">
							...
							<div class="input_ph">
								<input type="text" class="input_txt" id="depdt1_1" name="depdt1_1" placeholder="출발일자 선택" readonly />
							</div>

						<!-- 
							<div class="input_ph">
								<input type="text" id="depdt1_1" name="depdt1_1" class="input_txt hasDatepicker" 
								placeholder="출발일자 선택" readonly />
							</div>
							 -->
							<!-- 
							<select class="airportfind-content-inside">
								<option>인천/김포</option>
							</select> -->
						...
						<div class="quickDiv clearfix relative">
							<!-- 클릭 시 모달창이나 팝업창 띄워 검색하기 기능 -->
							
							<div class="input_ph">
								<input type="text" id="airSearchcity" name="airSearchcity" class="input_txt openairSearchPopWrap" placeholder="도착도시 검색" readonly />
							</div>
							<div class="input_ph whenyoucome" id="arrDtBox" style="display: block;">
								<input type="text" id="depdt2_1" name="depdt2_1" class="input_txt" placeholder="도착일자 선택" onclick="fn_depdt2Check(1);" readonly />
							</div>
							<!-- 
							<input type="text" id="depdt2_1" name="depdt2_1" readonly class="airportfind-content-inside whenyoucome" onclick="fn_depdt2Check(1);"	placeholder="도착일자 선택" />
							 -->
							<!-- 
							<input type="text" class="airportfind-content-inside openairSearchPopWrap" id="airSearchcity" name="airSearchcity"placeholder="도착도시 검색" readonly/>
							 -->
							 
							<div class="air_date_list dontknowWhen" id="opencaseY" style="display: none;">
								<div class="select_bx_st st01 w100">
									<label for="select_bx_st">체류일 선택</label>
									<select id="openday_init">
										<option value="">체류일</option>
						...
						<div class="quickDiv">인원</div>
						<div class="quickDiv clearfix">
								<!-- select id 성인 :adult 소아 :child 유아 : baby  -->
								
								
								
								<div class="sel_boxs w100">
									<div class="select_bx_st st02" style="width:33.3%">
										<label for="prd_sel06">성인1명</label>
										<select id="adult">
			...
			<a href="#" class="btn_m_pop_close" style="display: block;"><img src="/images/common/btn_pop_close.png" alt="여행상품 및 항공권찾기 팝업 닫기"></a>
			<!-- <a href="#" id="quickCloseBtn">X</a> -->
		</div>
	</div>
	<div class="sliderkit carousel mainb">
	    
	    	<div class="sliderkit-btn sliderkit-go-btn sliderkit-go-prev"><a href="#"><img src="/images/main/btn/btn_mainbanner_prev.gif" alt="이전"></a></div>
	    	<div class="sliderkit-btn sliderkit-go-btn sliderkit-go-next"><a href="#"><img src="/images/main/btn/btn_mainbanner_next.gif" alt="다음"></a></div>
	    	<div class="sliderkit-nav">
				<div class="sliderkit-nav-clip" style="width: 1020px; height: 40px;">
				<ul class="tabs" style="width: 1020px;">
						<li class="" style="width: 204px; height: 40px;">
							<a href="/product/locList.do?menu=pkg&amp;did=215">
									나는 가만히 바라볼 뿐이다</a>
							</li>
						<li style="width: 204px; height: 40px;" class="">
							<a href="/promotion/Overseas/2017/0921_as_laos/">
									마음속으로만 살고 있던 사람이</a>
							</li>
						<li style="width: 204px; height: 40px;" class="">
							<a href="/product/unitList.do?menu=pkg&amp;did=8399&amp;goodsCd=AVP4032">
									다낭, 블루 라방더</a>
							</li>
						<li style="width: 204px; height: 40px;" class="sliderkit-selected">
							<a href="/product/unitList.do?menu=pkg&amp;did=731&amp;goodsCd=EWP1116">
									스위스 미처 못다 한 이야기</a>
							</li>
						<li style="width: 204px; height: 40px;" class="">
							<a href="/promotion/Overseas/2016/0920_gs_nikko/">
									정말 쉬러 온 나</a>
							</li>
						</ul>
				</div>
	    ...
	    <div class="sliderkit-panels">	
	        <div class="sliderkit-panel">
	            <a href="javascript:;"><img alt="1번째 이미지" src="/images/tp-main/newslider01.jpg"></a>
	        </div>
	        <div class="sliderkit-panel">
	            <a href="javascript:;"><img alt="1번째 이미지" src="/images/tp-main/newslider02.jpg"></a>
	        </div>
	        <div class="sliderkit-panel">
	            <a href="javascript:;"><img alt="1번째 이미지" src="/images/tp-main/newslider03.jpg"></a>
	        </div>
	        <div class="sliderkit-panel">
	            <a href="javascript:;"><img alt="1번째 이미지" src="/images/tp-main/newslider04.jpg"></a>
	        </div>
	        <div class="sliderkit-panel">
	            <a href="javascript:;"><img alt="1번째 이미지" src="/images/tp-main/newslider05.jpg"></a>
	        </div>
	    </div>         
	</div>
